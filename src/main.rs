...
fn read_path(path: &str, projection: Option<&[usize]>) -> Result<RecordBatch> {
    ...
    let mut reader = read::ReaderBuilder::new().delimiter(b'|').from_path(path)?;
...
// Usage:
//    % datautils -from A.csv -from_opts header=Y separator='|' -to A.parquet -to_opts
...
fn main() -> Result<()> {
    ...
    let file_path = "/Users/adarshrp/Projects/tpch-data/sf0.01/customer.tbl";
